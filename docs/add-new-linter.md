# How to add support for a new tool to awesome-linter
...
4. Update expected summary reports: `test/data/awesome-linter-summary`.
...
- `test/inspec/awesome-linter/controls/awesome_linter.rb`
...
- Install the tool by pointing to specific package or container image versions:
  ...
  - If there are PyPi packages:
    ...
    1. Add the new virtual environment `bin` directory to the `PATH` in the
       Awesome-linter `Dockerfile`, in the `Configure Environment` section.
       Example:
  ...
  - If there are Maven or Java packages:
    ...
    2. Create a `dependencies/<name-of-tool>/build.gradle` file with the
       ...
       group 'com.github.awesome-linter'
...
- Update the orchestration scripts to run the new tool:
  ...
  - Define the command to invoke the new tool:
    ...
    - `lib/globals/linterCommandsOptions.sh`: add "check only mode" and "fix
      ...
      Awesome-linter will automatically add them to the command to run the tool.
...
1. Define a new variable in `lib/globals/linterRules.sh`:
   ...
   Awesome-linter automatically initializes the path to the configuration file in
...
### Configure the new tool for the Awesome-linter repository

If the default configuration of the new tool is unsuitable for the Awesome-linter
...
- Modify the core Awesome-linter logic: `lib/linter.sh`
