# Awesome-linter upgrade guide

This document helps you upgrade from a awesome-linter version to newer ones:
...
This section helps you upgrade from awesome-linter `v6` to `v7`.
...
reason, Awesome-linter `v7` doesn't include sql-lint anymore. If you are using
sql-lint to check your SQL files, we recommend that you evaluate
[sqlfluff](https://sqlfluff.com/), included in Awesome-linter since `v4.6.0`.
...
This section helps you upgrade from awesome-linter `v6.7.0` to `v6.8.0`.
...
- The `JAVASCRIPT_DEFAULT_STYLE` and `TYPESCRIPT_DEFAULT_STYLE` have been
  ...
  behavior of how
  [`VALIDATE_xxx` variables work](https://github.com/khulnasoft-lab/awesome-linter?tab=readme-ov-file#configure-awesome-linter).
  ...
  - If you're explicitly enabling linters and formatters that you want
    Awesome-linter to run by setting `VALIDATE_xxxx` variables to `true`:
  ...
  - If you're explicitly disabling linters and formatters that you don't want
    Awesome-linter to run by setting `VALIDATE_xxxx` variables to `false`:
  ...
  from your Awesome-linter configuration.

## Upgrade from v5 to v6

This section helps you upgrade from awesome-linter `v5` to `v6`.
...
- eslint-config-airbnb-typescript (a library to add TypeScript support to
  ...
  We had to remove it from awesome-linter because it was blocking updates to other
...
- awesome-linter doesn't include a default configuration file for `dart analyzer`
...
- `ERROR_ON_MISSING_EXEC_BIT` has been deprecated to align the
  ...
  `ERROR_ON_MISSING_EXEC_BIT` variable from your awesome-linter configuration.
...
- Setting `USE_FIND_ALGORITHM` to `true` and `VALIDATE_ALL_CODEBASE` to `false`
  is an unsupported configuration. awesome-linter `v5` and earlier silently
  ignored `VALIDATE_ALL_CODEBASE` when `USE_FIND_ALGORITHM` is set to `true`,
  leading to potentially confusing behavior for users. awesome-linter `v6`
...
- The `VALIDATE_KOTLIN_ANDROID` variable has been deprecated because ktlint
  ...
  awesome-linter doesn't need to account for this special case anymore. If you set
