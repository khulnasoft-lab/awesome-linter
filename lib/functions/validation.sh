...
function ValidateBooleanConfigurationVariables() {
  ...
  ValidateBooleanVariable "SAVE_AWESOME_LINTER_OUTPUT" "${SAVE_AWESOME_LINTER_OUTPUT}"
  ValidateBooleanVariable "SAVE_AWESOME_LINTER_SUMMARY" "${SAVE_AWESOME_LINTER_SUMMARY}"
...
function ValidateFindMode() {
  ...
  if [[ "${USE_FIND_ALGORITHM}" == "true" ]] && [[ "${VALIDATE_ALL_CODEBASE}" == "false" ]]; then
    error "Setting USE_FIND_ALGORITHM to true and VALIDATE_ALL_CODEBASE to false is not supported because awesome-linter relies on Git to validate changed files."
    return 1
  fi
}
...
function ValidateAwesomeLinterSummaryOutputPath() {
  debug "Validating AWESOME_LINTER_SUMMARY_OUTPUT_PATH"
  if [[ -z "${AWESOME_LINTER_SUMMARY_OUTPUT_PATH:-}" ]]; then
    error "AWESOME_LINTER_SUMMARY_OUTPUT_PATH is not set."
    return 1
  fi
  debug "AWESOME_LINTER_SUMMARY_OUTPUT_PATH is set to: ${AWESOME_LINTER_SUMMARY_OUTPUT_PATH}"
  if [[ ! -e "${AWESOME_LINTER_SUMMARY_OUTPUT_PATH}" ]]; then
    error "AWESOME_LINTER_SUMMARY_OUTPUT_PATH (${AWESOME_LINTER_SUMMARY_OUTPUT_PATH}) doesn't exist."
    return 1
  fi
  if [[ ! -f "${AWESOME_LINTER_SUMMARY_OUTPUT_PATH}" ]]; then
    error "AWESOME_LINTER_SUMMARY_OUTPUT_PATH (${AWESOME_LINTER_SUMMARY_OUTPUT_PATH}) is not a file."
    return 1
  fi
  debug "Awesome-linter summary ouput path passed validation"
}
...
function WarnIfVariableIsSet() {
  ...
  if [ -n "${INPUT_VARIABLE:-}" ]; then
    warn "${INPUT_VARIABLE_NAME} environment variable is set, it's deprecated, and awesome-linter will ignore it. Remove it from your configuration. This warning may turn in a fatal error in the future. For more information, see the upgrade guide: https://github.com/khulnasoft-lab/awesome-linter/blob/main/docs/upgrade-guide.md"
  fi
}
