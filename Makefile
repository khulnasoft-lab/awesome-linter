...
info: ## Gather information about the runtime environment
	...
	echo "AWESOME_LINTER_TEST_CONTAINER_URL:" $(AWESOME_LINTER_TEST_CONTAINER_URL); \
	echo "ls -ahl: $$(ls -ahl)"; \
	docker images; \
	docker ps; \
...
.PHONY: docker-build-check ## Run Docker build checks against the Awesome-linter image
docker-build-check:
...
open-shell-awesome-linter-container: ## Open a shell in the Awesome-linter container
	docker run $(DOCKER_FLAGS) \
		...
		--rm \
		-v "$(CURDIR)":/tmp/lint \
		-v "$(CURDIR)/ci-tools/Gemfile.lock":/Gemfile.lock \
		-v "$(CURDIR)/ci-tools/Gemfile":/Gemfile \
		-v "$(CURDIR)/ci-tools/package-lock.json":/package-lock.json \
		-v "$(CURDIR)/ci-tools/package.json":/package.json \
		-v "$(CURDIR)/ci-tools/composer/composer.json":/php-composer/composer.json \
		-v "$(CURDIR)/ci-tools/composer/composer.lock":/php-composer/composer.lock \
...
.PHONY: test-lib
test-lib: test-globals-languages test-linter-rules test-build-file-list test-detect-files test-github-event test-setup-ssh test-validation test-output test-linter-commands ## Test awesome-linter libs and globals
...
open-shell-dev-container: build-dev-container-image ## Open a shell in the dev tools container
	docker run $(DOCKER_FLAGS) \
		...
		--entrypoint /bin/bash \
		--rm \
		-v "$(CURDIR)/dev/package-lock.json":/package-lock.json \
		-v "$(CURDIR)/dev/package.json":/package.json \
		$(DEV_CONTAINER_URL)
